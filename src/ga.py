...
# Protected operators and output sanitation
...
def build_toolbox(n_features: int, seed: int, max_tree_height: int):
    ...
    # DEAP creators (guarded against redefinition)
    ...
    # GA operators (selection is overridden later to use tournament_size)
...
def _query_indices_random(rng: np.random.Generator, pool_size: int, n_query: int) -> np.ndarray:
    return rng.choice(pool_size, size=n_query, replace=False)
...
def _active_learning_step(
    *,
    ...
    n_query = min(int(al_samples_per_round), pool_x.shape[0])
    if n_query <= 0:
        return pool_x, pool_y

    # Use current best individual as "query model" (simple, consistent, defensible)
    best = tools.selBest(population, 1)[0]

    if al_strategy == "random":
        q_idx = _query_indices_random(rng, pool_x.shape[0], n_query)
    elif al_strategy == "uncertainty":
        q_idx = _query_indices_uncertainty(rng, best, toolbox, pool_x, n_query)
    else:
        raise ValueError(
            f"Unknown al_strategy '{al_strategy}'. Choose from {'uncertainty','random'}."
        )

    # Move queried samples from pool to labeled set
    data["x"] = np.vstack([data["x"], pool_x[q_idx]])
    data["y"] = np.concatenate([data["y"], pool_y[q_idx]])

    keep = np.ones(pool_x.shape[0], dtype=bool)
    keep[q_idx] = False
    pool_x = pool_x[keep]
    pool_y = pool_y[keep]
...
def run_ga(
    ...
    *,
    ...
    if active_learning:
        init_size = max(1, int(len(x_train) * float(al_initial_fraction)))
    ...
    # Bind evaluation to current data dict (which will grow with AL)
    ...
    # IMPORTANT: evaluate initial population once (robust DEAP workflow)
    ...
    for gen in range(1, int(generations) + 1):
        # Active Learning step (every al_interval generations)
        if active_learning and int(al_interval) > 0 and (gen % int(al_interval) == 0):
            pool_x, pool_y = _active_learning_step(
                rng=rng,
                al_strategy=str(al_strategy),
                al_samples_per_round=int(al_samples_per_round),
                toolbox=toolbox,
...
def _evaluate_individual(individual, toolbox, data):
    ...
    outputs = _sanitize(outputs)
    preds = (outputs > 0).astype(int)
    return (f1_score(data["y"], preds, zero_division=0),)
...
def _query_indices_uncertainty(individual, toolbox, pool_x, n_query):
    func = toolbox.compile(expr=individual)
    scores = _sanitize(func(*pool_x.T))
    idx = np.argsort(np.abs(scores))[:n_query]
    return idx
...
def run_ga(
    ...
    *,
    ...
    for gen in range(1, generations + 1):
        ...
        if active_learning and gen % al_interval == 0 and pool_x is not None:
            best = tools.selBest(population, 1)[0]
            n_query = min(al_samples_per_round, len(pool_x))
            idx = _query_indices_uncertainty(best, toolbox, pool_x, n_query)
            ...
            mask[idx] = False
            pool_x, pool_y = pool_x[mask], pool_y[mask]
        ...
        print(
            ...
            f"Train size={row['train_size']}"
        )

    return GAResult(population, best, log, time.time() - start)
