...
points à corriger : 
rien ne s'affiche dans /data. 
le weighted voting s'execute dans le pipeline, c'est seulement censé se mettre pour le soft voting, et on est d'accord que c'est seulement ça (pour AL+EL+GA) qui est implémenté dans le script suivant : "from __future__ import annotations
...
"
il faut que le report pour GA+AL+EL soit affiché dans les logs comme pour GA et GA+AL.
De plus, je pense qu'il est nécessaire de faire en sorte de ne pas avoir à rerun les approches déjà faites si elles ont déjà été run. Dans le sens où il faut juste append à ce uqi est déjà présent les expériments pour ensuite stocker dans experiments ce qui n'a pas été fait dans la pipeline en prod.
Pas besoin de refaire le pre-processing, vu qu'il est déjà fait au début de la pipeline. experiments est une brique qu'on rajoute à la pipeline si son booléen d'activation est true.